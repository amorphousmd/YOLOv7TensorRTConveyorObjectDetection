...
class BaseEngine(object):
    def __init__(self, engine_path, imgsz=(320, 320), use_torch=False, use_onnx=False):
        ...
        self.input_shapes = [[d.dim_value for d in _input.type.tensor_type.shape.dim] for _input in
    ...
    def PreProcess(self, image_path):
        ...
        if (np.shape(image) != self.input_shapes[0][
                               2:4]):  # Not the same shape as the input of the onnx model, needs to implement dynamical shape
    ...
    def PostProcess(self, img, hyp, inf_out, attn, bases, sem_output, real_image):
        ...
        output, output_mask = non_max_suppression_mask_conf(inf_out, attn, bases, pooler, hyp, conf_thres=0.25,
                                                            iou_thres=0.65, merge=False, mask_iou=None)
    ...
    def inference(self, image_path, conf=0.25):
        ...
        for img_path in tqdm.tqdm(image_path):
            ...
            if (np.shape(image)[0:2] != self.input_shapes[0][
                                        2:4]):  # Not the same shape as the input of the onnx model, needs to implement dynamical shape
                print("/!\ Shape of the input " + str(
                    np.shape(image)[0:2]) + " different from the input size of the ONNX model " + str(
                    self.input_shapes[0][2:4]) + ", have to resize the image.")
            ...
            bases = torch.reshape((output[0]),
                                  (1, 4, ((len(output[0]) // (self.imgsz[0] // 4)) // 4), (self.imgsz[0] // 4)))
            sem_output = torch.reshape((output[1]),
                                       (1, 1, (len(output[1]) // (self.imgsz[0] // 4)), (self.imgsz[0] // 4)))
            pnimg, nimg, real_image = self.PostProcess(img, hyp, inf_out, attn, bases, sem_output, real_image)

            if args.save_image:
                Path(args.save_path).mkdir(parents=True, exist_ok=True)
                print(" Saved in : " + str(args.save_path) + str(int(self.imgsz[0])) + "_trt_cv2img_VP_" + str(
                    iteration) + ".jpg")
                cv2.imwrite(str(args.save_path) + str(int(self.imgsz[0])) + "_trt_cv2img_VP_" + str(iteration) + ".jpg",
                            pnimg)
            iteration += 1
        if args.save_image:
