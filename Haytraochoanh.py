...
class Yolov7(object):
    ...
    def inference(self, origin_img):
        ...
        img /= 255
        self.time_pre_pro = time.perf_counter() - start
        
        start = time.perf_counter()
        num, final_boxes, final_scores, final_cls_inds = self.infer(img)
        
        self.time_detect = time.perf_counter() - start
        start = time.perf_counter()
        final_boxes = np.reshape(final_boxes, (-1, 4))
        num = num[0]
        self.time_post_pro = time.perf_counter() - start
        if num > 0:
...
if __name__ == '__main__':
    ...
    Yolov7_model = Yolov7(engine_path='yolov7-tiny-nms.trt')
    Yolov7_model.get_fps()
    image = cv2.imread('3.png')
    # image, r = preproc(image, (640, 640), mean=None, std=None)

    thread1 = myThread(Yolov7_model.inference, image)
    thread1.start()
    thread1.join()
    # data = Yolov7.infer(image)
    print(f'time pre:{Yolov7_model.time_pre_pro}, time detect:{Yolov7_model.time_detect}, time post:{Yolov7_model.time_post_pro}')
    Yolov7_model.destory()
    print('end___')