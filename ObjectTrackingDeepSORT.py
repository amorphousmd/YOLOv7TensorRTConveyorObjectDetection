...
class YoloDetector():
    ...
    def load_model(self, model_name):
        ...
        if model_name:
            model = torch.hub.load('ultralytics/yolov5', 'custom', path='./YOLOv5sCandyVer3.pt')
        else:
            model = torch.hub.load('ultralytics/yolov5', 'custom', path='./YOLOv5sCandyVer3.pt')
        return model
    ...
    def plot_boxes(self, results, frame, height, width, confidence=0.3):
        ...
        for i in range(n):
            ...
            if row[4] >= confidence:
                ...
                if self.class_to_label(labels[i]) == 'Keo':
                    ...
                    detections.append(([x1, y1, int(x2-x1), int(y2-y1)], row[4].item(), 'Keo'))
        print(detections)
